  ...
  return (
    <div style={{ background:'#fff', borderRadius:14, border:'1px solid #e4eaf3', boxShadow:'0 1px 4px rgba(15,23,42,0.06)' }}>
      ...
      <div style={{ padding:'10px 16px 16px', maxHeight:340, overflowY:'auto' }}>
        {inventory.length === 0 ? (
          <div style={{ textAlign:'center', padding:40, color:'#94a3b8', fontSize:13 }}>لا توجد منتجات في المخزون</div>
        ) : inventory.map(item => {
          const isLow = item.stock < 5
          const pct   = Math.min(100, (item.stock / Math.max(item.stock + 5, 15)) * 100)
            ...
            <div key={item.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 8px', borderRadius:10, marginBottom:4, background: isLow ? '#fff1f2' : 'transparent', border:`1px solid ${isLow ? '#fecdd3' : 'transparent'}` }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                  ...
                  {isLow && <AlertTriangle size={11} color="#f43f5e" style={{ flexShrink:0 }} />}
                </div>
              ...
              <div style={{ textAlign:'left', flexShrink:0 }}>
                <div style={{ fontSize:13, fontWeight:700, color: isLow ? '#e11d48' : '#0f172a' }} dir="ltr">
                  {item.stock} <span style={{ fontSize:11, fontWeight:400, color:'#94a3b8' }}>وحدة</span>
                </div>
                {displayPrice > 0 && (
                  <div style={{ fontSize:11, color:'#94a3b8' }} dir="ltr">{displayPrice.toLocaleString()} LE</div>
                )}
              </div>
            </div>
          )
