...
export default function StatsCards() {
  ...
  const sales   = orders.filter(o => o.status !== 'مرفوض').reduce((s, o) => s + o.total, 0)
  ...
  const cards = [
    {
      ...
      value: total,
      sub: `${closed} تم التحصيل`,
      badge: `+${monthN} هذا الشهر`,
      iconBg: 'linear-gradient(135deg,#2563eb,#1d4ed8)',
      tint: '#eff6ff', tintBorder: '#bfdbfe', valColor: '#1d4ed8',
    },
    {
      icon: TrendingUp,
      ...
      suffix: 'LE',
      sub: 'إجمالي القيمة',
      badge: `${thisMonth.filter(o => o.status !== 'مرفوض').reduce((s, o) => s + o.total, 0).toLocaleString()} LE هذا الشهر`,
      iconBg: 'linear-gradient(135deg,#7c3aed,#6d28d9)',
      tint: '#f5f3ff', tintBorder: '#ddd6fe', valColor: '#6d28d9',
    },
    {
      icon: CalendarDays,
      label: 'أوردرات الأسبوع',
      value: week,
      sub: 'آخر 7 أيام',
      badge: `${monthN} هذا الشهر`,
      iconBg: 'linear-gradient(135deg,#059669,#047857)',
      tint: '#ecfdf5', tintBorder: '#a7f3d0', valColor: '#047857',
    },
    {
      icon: Target,
      label: 'معدل الإغلاق',
      value: `${rate}%`,
      sub: `${closed} من ${total} طلب`,
      badge: `${total > 0 ? Math.round((thisMonth.filter(o => o.status === 'تم التحصيل').length / Math.max(thisMonth.length, 1)) * 100) : 0}% هذا الشهر`,
      iconBg: 'linear-gradient(135deg,#0891b2,#0e7490)',
      tint: '#ecfeff', tintBorder: '#a5f3fc', valColor: '#0e7490',
    },
  ]
      ...
      {cards.map(c => (
        <div key={c.label} style={{ background:'#fff', borderRadius:14, border:'1px solid #e4eaf3', padding:'20px 22px', boxShadow:'0 1px 4px rgba(15,23,42,0.06)' }}>
          <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:16 }}>
            ...
            <div style={{ padding:'4px 10px', borderRadius:20, background:c.tint, border:`1px solid ${c.tintBorder}`, fontSize:10, color:c.valColor, fontWeight:700, textAlign:'center', maxWidth:130 }}>
              {c.badge}
            </div>
          </div>
